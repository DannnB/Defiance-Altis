	...
	while {true} do
	{
	    ...
	    _updatedMarkers = [];
	    {
			...
			if ((_side == playerSide || _isSpectator) && _x != _spectator) then {
				...
				_marker = _x getVariable ["playerMarker",nil];
				if (isNil "_marker") then
				{
					_marker = createMarkerLocal [(format["pm_%1",_playerUID]),[0,0]];
					_x setVariable ["playerMarker", _marker];
...
cl_bluforLoop = {
	...
	{
		_flag = "Flag_Blue_F" createVehicleLocal (getMarkerPos _x);
		id = _flag addAction ["Travel to MHQ","player setPos ((getPos b_mhq) findEmptyPosition [5,100])"];
		if (_airfieldActive) then
		{
			id = _flag addAction ["Travel to HQ","player setPos ((getMarkerPos 'respawn_west') findEmptyPosition [5,100])"];
			id = _flag addAction ["Travel to Airfield","player setPos ((getMarkerPos 'respawn_west2')  findEmptyPosition [5,100])"];
		};
...
//CHECK FOR VAS
	...
	deleteMarkerLocal "respawn_west2";

...
sv_handleTKer = {
	...
	{
		private ["_TKIndex","_lastTime","_warnLevel"];
		...
		if (_lastTime+1 < time) then
		{
			_warnLevel = _warnLevel+1;
			if (_warnLevel > 2) then {TK_PLAYERUIDS set [count TK_PLAYERUIDS,_TKuid]};
			if (_TKIndex >=0) then {
		 		TK_TRACKING set [_TKIndex+1,[time,_warnLevel]];
	     	}
	     	else
	     	{
	     		_count = count TK_TRACKING;
	     		TK_TRACKING set [_count,_TKuid];
	     		TK_TRACKING set [_count+1,[time,1]];
	     	};

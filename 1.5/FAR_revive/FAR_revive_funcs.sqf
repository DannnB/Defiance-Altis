...
FAR_Player_Unconscious =
{
	...
	if (isPlayer _unit) then
	{
		...
		//Nash Edit this works
		if (side _unit == west) then { _tickets = [west,-1] call BIS_fnc_respawnTickets};
	};
	...
	if (isPlayer _unit) then
	{
		...
		else
		{
			...
			//Nash Edit - Restore ticket to BLUFOR
			if (side _unit == west) then { _tickets = [west,1] call BIS_fnc_respawnTickets};
		};
	}
	else
...
FAR_HandleRevive =
{
	...
	if (alive _target) then
	{
		...
		if (side _target == west) then { _tickets = [west,1] call BIS_fnc_respawnTickets};

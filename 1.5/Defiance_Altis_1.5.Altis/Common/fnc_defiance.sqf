...
cmn_AddKarts = {
	if (isServer) then
	{
		b_zeus addCuratorAddons ["A3_Soft_F_Kart_Kart_01"];
		i_zeus addCuratorAddons ["A3_Soft_F_Kart_Kart_01"];
		b_atc addCuratorAddons ["A3_Soft_F_Kart_Kart_01"];
	};
	systemchat "DEFIANCE: Karts have been enabled. Commanders can now spawn Karts for ALL players to use.";
};
		...
		if (!isNil "_unit") then
		{
			if (!isNull _unit) then
			{
				_unit addEventHandler ["Fired",{
					[_this] spawn
					{
						_mortar = (_this select 0) select 0;
						_projectile = (_this select 0) select 6;
						//if (player == gunner _mortar) then {
						if (vehicle player == _mortar) then {
							_base = getMarkerPos "respawn_west";
							while {!isNull _projectile} do
							{
								_dis = _projectile distance _base;
								//systemChat str(_dis);
								if (_dis < 150) then
								{
									deleteVehicle _projectile;
									hintSilent "Mortar round deleted. Please do not shell the BLUFOR HQ Spawn";
								};
								sleep 0.5;
							};
						};
...
cmn_TeamLocked = {
	...
	systemchat format ["%1 has tried to team switch but is locked to %2", _pname, [_sideLocked] call bis_fnc_sideName];
...
cmn_RefundTKTickets = {
	...
	systemchat format ["BLUFOR have been refunded %1 tickets as a result of %2 team killing",_tickets,_pname ];
};

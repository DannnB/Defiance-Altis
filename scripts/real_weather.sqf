	...
	*/
	...
	// Mission starting from param edit by Nash
	_startingdate = _this select 0;

	// Mission starting weather "CLEAR|CLOUDY|RAIN";
	_weatherArray = ["CLEAR","CLOUDY","RAIN"];
	_weatherselected = _this select 1;
	if (_weatherselected < 0) Then
	{
		_weatherselected = floor (random 3);
	};
	_startingweather = _weatherArray select _weatherselected;
